...
def gerando_dataframe(dados_limpos, df, ajust):
    if df is None:
        ...
        df['mercadoria'] = df['codigo_mercadoria'].astype(int).map(commodity_dict)
        ...
        df_novos['mercadoria'] = df_novos['codigo_mercadoria'].astype(int).map(commodity_dict)
...
print(df_forn.dtypes)
df_forn.to_excel(ROOT_PATH / 'Planilhas_tratadas'/ 'Fornecimento.xlsx', index=False)
wb_forn = openpyxl.load_workbook(ROOT_PATH / 'Planilhas_tratadas'/  'Fornecimento.xlsx')  
ws_forn = wb_forn.active

ajustar_colunas(ws_forn)
ajustar_bordas(wb_forn)
wb_forn.save(ROOT_PATH / 'Planilhas_tratadas'/  'Fornecimento.xlsx')
